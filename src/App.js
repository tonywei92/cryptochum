...
function App() {
  ...
  useEffect(() => {
      ...
      function (request, sender, sendResponse) {
        ...
        if (request.hasOwnProperty('type')) {
          ...
          if (request.type === messageConstants.ADDRESS) {
            setUserAddress(request.address)
          }
    ...
    sendMessage({ type: messageConstants.REQUEST_ADDRESS })
      ...
      <div className="menu top-0 right-0 text-sm transition-all">
        <div className="font-medium mb-2">Cryptochum</div>
        {
          userAddress && character ? (
            <>
              ...
              <div className="flex -mx-2 items-center mb-2">
                <div className="px-2 flex justify-center items-center w-full">
                  <div className={`char-canvas-large char-canvas-large__idle my-4`} style={
                    {
            ...
            <div>
              <button className="bg-yellow-500 text-white px-4 py-2 transition-all rounded border border-yellow-600 hover:border-yellow-800 border-b-4" onClick={() => {
                if (chrome) {
                  chrome.tabs.create({ active: true, url: "https://cryptochum-web.vercel.app/" });
                }
              }}>
                Login
              </button>
